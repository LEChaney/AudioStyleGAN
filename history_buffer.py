...
class HistoryBuffer(object):
    def __init__(self, clip_length, max_size, batch_size):
        ...
        """
        self.history_buffer = tf.get_variable('history_buffer', initializer=tf.zeros([max_size, clip_length + 1024, 1]), trainable=False, dtype=tf.float32)
    ...
    def add_to_history_buffer(self, audio_clips, cond_embeds, nb_to_add=None):
        """
        ...
        def not_full_fn():
            ...
            slice_assign_op = self.history_buffer[slice_start:slice_end, 0:self.clip_length].assign(audio_clips[:nb_to_add])
            with tf.control_dependencies([slice_assign_op]):
                slice_assign_op = self.history_buffer[slice_start:slice_end, self.clip_length:].assign(cond_embeds[:nb_to_add])
                with tf.control_dependencies([slice_assign_op]):
        ...
        def full_fn():
            slice_assign_op = self.history_buffer[:nb_to_add, 0:self.clip_length].assign(audio_clips[:nb_to_add])
            with tf.control_dependencies([slice_assign_op]):
                slice_assign_op = self.history_buffer[:nb_to_add, self.clip_length:].assign(cond_embeds[:nb_to_add])
    ...
    def get_from_history_buffer(self, nb_to_get=None):
        ...
        return (self.history_buffer[:nb_to_get, 0:self.clip_length],
                tf.squeeze(self.history_buffer[:nb_to_get, self.clip_length:]))